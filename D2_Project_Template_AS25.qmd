...
::: {style="color: green;"}
Yes, to minimize the effect of temporary network fluctuations or time-dependent variables. The run order was randomized for each person.
:::

Is there anything else to consider?

::: {style="color: green;"}
Ensure no heavy background tasks (like App Updates, Backups) are running during the test, to avoid blocking factors. Also as 'Home' as well as 'Uni' refers to different physical locations for each person, we treat 'Person' as a blocking factor in the analysis. This accounts for the different baseline internet speeds of our respective locations
:::

Is there anything which is not ideal in terms of experimentation, but where you did not find a better solution?

::: {style="color: green;"}
Exact distance to the router was sometimes difficult to determine (especially at uni), so we used the best possible estimate.
:::
...
::: {style="color: green;"}
The model explains a large portion of the variance (Adjusted R-squared = 0.84), indicating a good fit. The Residuals vs Fitted plot shows no strong pattern, suggesting linearity. The Q-Q plot shows residuals are roughly normal, despite some deviation at the tails. There are no extreme outliers.
:::

[**Altogether we have found out that:**]{style="color: green;"}

::: {style="color: green;"}
Distance to the router is the strongest influence factor: moving from 1m to 5m drops speed by \~339 Mbps, and to 10m by \~587 Mbps. Device is also significant, with phones being slower (\~161 Mbps less) than laptops. Location was weakly significant (p=0.04), while Person did not have a statistically significant impact on the speed in our model.
:::

#### Step 6: Final Comments

```{r}
model_int <- lm(download_speed_mbit_result ~ (person + device + location + approx_distance_router_meters)^2, data = df)
...
::: {style="color: green;"}
The ANOVA test comparing the additive model to the model with interactions yields a p-value of \< 2.2e-16. This is highly significant, meaning that the factors do not act independently. The effect of one factor (e.g. distance) likely depends on the level of another factor (e.g. device or location), and the interaction model describes the data significantly better.
:::
